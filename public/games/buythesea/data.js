...
const UPGRADE_CATEGORIES = [
  ...
  { name: 'ABILITIES',     roots: ['bait_bomb', 'multi_cast', 'boss_bomb', 'harpoon_hammer'] },
...
const UPGRADES_DEF = [
  ...
  { id: 'harpoon_damage_3',   name: 'HARPOON DAMAGE III',desc: 'Greatly increases harpoon damage.',           baseCost: 80000, max: 10, costMult: 1.3, tierOf: 'harpoon_damage', parent: 'harpoon_damage_2' },
  ...
  { id: 'harpoon_speed_3',    name: 'HARPOON SPEED III', desc: 'Greatly increases harpoon cast speed.',       baseCost: 120000,max: 10, costMult: 1.3, tierOf: 'harpoon_speed', parent: 'harpoon_speed_2' },
  ...
  { id: 'boss_bomb',        name: 'BOSS BOMB',        desc: 'Instantly deal 30% of boss max HP during a boss fight.', baseCost: 15000, max: 1 },
  { id: 'boss_bomb_cooldown', name: 'BOSS BOMB EFFICIENCY', desc: 'Reduces Boss Bomb cooldown by 20s.', baseCost: 12000, max: 5, parent: 'boss_bomb' },
  { id: 'harpoon_hammer',   name: 'HARPOON HAMMER',   desc: 'Harpoon deals 2x damage for 10s.',             baseCost: 20000, max: 1 },
  { id: 'hammer_cooldown',  name: 'HAMMER EFFICIENCY', desc: 'Reduces Harpoon Hammer cooldown by 15s.',     baseCost: 15000, max: 5, parent: 'harpoon_hammer' },
  { id: 'hammer_duration',  name: 'HAMMER EXTEND',    desc: '+3s Harpoon Hammer duration.',                  baseCost: 18000, max: 5, parent: 'harpoon_hammer' },
...
function getHarpoonDamage() {
  const base = 5;
  let dmg = base + getUpgradeLevel('harpoon_damage') * 3;
  if (state.harpoonHammerTimer > 0) dmg *= 2;
  return dmg;
}
...
const ACHIEVEMENTS = [
  {
    ...
    progress: () => ({ current: state.stats.totalCaught, target: 20 }),
    reward: { gold: 100 },
    rewardDesc: '100 ✧',
  },
  {
    id: 'collect_100_fish', name: 'STEADY HANDS',
    desc: 'Catch 100 fish.',
    check: () => state.stats.totalCaught >= 100,
    progress: () => ({ current: state.stats.totalCaught, target: 100 }),
    reward: { gold: 500 },
    rewardDesc: '500 ✧',
  },
  {
    id: 'collect_500_fish', name: 'NET POSITIVE',
    desc: 'Catch 500 fish.',
    check: () => state.stats.totalCaught >= 500,
    progress: () => ({ current: state.stats.totalCaught, target: 500 }),
    reward: { gold: 2500 },
    rewardDesc: '2,500 ✧',
  },
  {
    id: 'collect_2000_fish', name: 'DEEP TRAWLER',
    desc: 'Catch 2,000 fish.',
    check: () => state.stats.totalCaught >= 2000,
    progress: () => ({ current: state.stats.totalCaught, target: 2000 }),
    reward: { gold: 10000 },
    rewardDesc: '10,000 ✧',
  },
  {
    id: 'collect_10000_fish', name: 'LEVIATHAN HARVESTER',
    desc: 'Catch 10,000 fish.',
    check: () => state.stats.totalCaught >= 10000,
    progress: () => ({ current: state.stats.totalCaught, target: 10000 }),
    reward: { gold: 50000 },
    rewardDesc: '50,000 ✧',
  },
  {
    id: 'earn_1000_gold', name: 'POCKET CHANGE',
    desc: 'Earn 1,000 gold total.',
    check: () => state.totalGoldEarned >= 1000,
    progress: () => ({ current: state.totalGoldEarned, target: 1000 }),
    reward: { gold: 200 },
    rewardDesc: '200 ✧',
  },
  {
    id: 'earn_50000_gold', name: 'LIQUID ASSETS',
    desc: 'Earn 50,000 gold total.',
    check: () => state.totalGoldEarned >= 50000,
    progress: () => ({ current: state.totalGoldEarned, target: 50000 }),
    reward: { gold: 5000 },
    rewardDesc: '5,000 ✧',
  },
  {
    id: 'earn_500000_gold', name: 'THE TREASURY',
    desc: 'Earn 500,000 gold total.',
    check: () => state.totalGoldEarned >= 500000,
    progress: () => ({ current: state.totalGoldEarned, target: 500000 }),
    reward: { gold: 25000 },
    rewardDesc: '25,000 ✧',
  },
  {
    id: 'defeat_first_boss', name: 'BOSS SLAYER',
    desc: 'Defeat your first boss.',
    check: () => state.bossesDefeated.length >= 1,
    progress: () => ({ current: state.bossesDefeated.length, target: 1 }),
    reward: { gold: 1000 },
    rewardDesc: '1,000 ✧',
  },
  {
    id: 'defeat_5_bosses', name: 'APEX PREDATOR',
    desc: 'Defeat 5 bosses.',
    check: () => state.bossesDefeated.length >= 5,
    progress: () => ({ current: state.bossesDefeated.length, target: 5 }),
    reward: { gold: 10000 },
    rewardDesc: '10,000 ✧',
  },
  {
    id: 'defeat_10_bosses', name: 'GOD KILLER',
    desc: 'Defeat 10 bosses.',
    check: () => state.bossesDefeated.length >= 10,
    progress: () => ({ current: state.bossesDefeated.length, target: 10 }),
    reward: { gold: 100000 },
    rewardDesc: '100,000 ✧',
  },
  {
    id: 'unlock_3_zones', name: 'EXPLORER',
    desc: 'Unlock 3 zones.',
    check: () => state.unlockedZones.length >= 3,
    progress: () => ({ current: state.unlockedZones.length, target: 3 }),
    reward: { gold: 800 },
    rewardDesc: '800 ✧',
  },
  {
    id: 'unlock_8_zones', name: 'DEEP DIVER',
    desc: 'Unlock 8 zones.',
    check: () => state.unlockedZones.length >= 8,
    progress: () => ({ current: state.unlockedZones.length, target: 8 }),
    reward: { gold: 15000 },
    rewardDesc: '15,000 ✧',
  },
  {
    id: 'buy_10_upgrades', name: 'LAB RAT',
    desc: 'Purchase 10 upgrades.',
    check: () => state.stats.totalUpgradesBought >= 10,
    progress: () => ({ current: state.stats.totalUpgradesBought, target: 10 }),
    reward: { gold: 300 },
    rewardDesc: '300 ✧',
  },
  {
    id: 'buy_50_upgrades', name: 'MAD SCIENTIST',
    desc: 'Purchase 50 upgrades.',
    check: () => state.stats.totalUpgradesBought >= 50,
    progress: () => ({ current: state.stats.totalUpgradesBought, target: 50 }),
    reward: { gold: 5000 },
    rewardDesc: '5,000 ✧',
  },
  {
    id: 'buy_150_upgrades', name: 'FULLY OPTIMIZED',
    desc: 'Purchase 150 upgrades.',
    check: () => state.stats.totalUpgradesBought >= 150,
    progress: () => ({ current: state.stats.totalUpgradesBought, target: 150 }),
    reward: { gold: 30000 },
    rewardDesc: '30,000 ✧',
  },
  {
    id: 'cast_500', name: 'CAST IRON ARM',
    desc: 'Cast your line 500 times.',
    check: () => state.stats.totalCasts >= 500,
    progress: () => ({ current: state.stats.totalCasts, target: 500 }),
    reward: { gold: 2000 },
    rewardDesc: '2,000 ✧',
  },
  {
    id: 'prestige_once', name: 'THE DEPTHS BECKON',
    desc: 'Prestige for the first time.',
    check: () => state.prestige.timesPrestiged >= 1,
    progress: () => ({ current: state.prestige.timesPrestiged, target: 1 }),
    reward: { gold: 5000 },
    rewardDesc: '5,000 ✧',
  },
  {
    id: 'prestige_5', name: 'ETERNAL RETURN',
    desc: 'Prestige 5 times.',
    check: () => state.prestige.timesPrestiged >= 5,
    progress: () => ({ current: state.prestige.timesPrestiged, target: 5 }),
    reward: { gold: 50000 },
    rewardDesc: '50,000 ✧',
  },
  {
    id: 'catalog_30', name: 'MARINE BIOLOGIST',
    desc: 'Discover 30 unique species in the catalog.',
    check: () => Object.keys(state.catalogCaught).length >= 30,
    progress: () => ({ current: Object.keys(state.catalogCaught).length, target: 30 }),
    reward: { gold: 8000 },
    rewardDesc: '8,000 ✧',
  },
];
