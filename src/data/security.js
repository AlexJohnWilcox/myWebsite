export const security = {
  ...
  posts: [
    {
      slug: 'tailscale-and-seven-exit-nodes',
      date: '2026-08-12',
      keywords: ['tailscale', 'wireguard', 'exit-nodes'],
      title: 'Tailscale and seven exit nodes',
      body: [
        `I moved into a new apartment and rebuilt my whole network from scratch, and the thing that bothered me most about the old setup was that everything I did left from the same VPN server every single day for months. One address that never changes is exactly the sort of thing I started this journey to get away from. So I generated seven separate WireGuard configs from Proton, all of them single hop servers in the US and Canada, and wrote a script that lives on my router and moves the tunnel to a different one every night somewhere between 3:00 and 3:15. What surprised me is that you can't rotate a VPN by just changing the address, each config comes with its own private key tied to one specific server, so switching exits means swapping the whole bundle at once. Before the script keeps a new exit it makes it prove itself, it checks for a fresh handshake, pings through the tunnel, sends a deliberately oversized packet to catch a path that quietly drops big ones, and resolves a name through the VPN's own resolver, and if any of that fails it puts the old exit back and quarantines the bad one so tomorrow doesn't pick it again.`,
        `The other half of this was figuring out how to get back in. My router has no path from my devices to the internet except through the tunnel, which is exactly what I wanted, but it also means there is nothing to connect to from outside, and I wasn't about to punch a hole in the firewall I'd just spent weeks building. After a lot of reading I landed on Tailscale, which builds its own encrypted mesh between my devices without exposing anything to the internet at all. I put it on my Pi and my phone, advertised my whole home subnet through it so I reach every machine and not just the one, and made the Pi an exit node so my phone can route all of its traffic back through my house when I'm on some coffee shop's WiFi. I also turned on Tailnet Lock, which stops Tailscale themselves from adding a device to my network unless one of my own machines signs for it first, and I set up two machines that can sign instead of one, because if the only signer died I'd never be able to enroll another device again.`,
        `Like everything else on this journey it came with a cost, and this one took me about ten days to find. My texts stopped working over cellular, and only my texts, every other app on the phone was completely fine, so I spent over a week blaming my carrier and my own ad blocker before I found the real answer. Android's block connections without VPN setting forces Google Messages to bypass the VPN, then drops the very traffic it just pushed outside, so the safety toggle was killing the one app it excluded. Turning it off fixed it immediately, and it was a good reminder that a setting that sounds protective isn't protecting anything until you've checked what it actually covers. Besides that I'm thrilled with it, my traffic leaves from somewhere different every morning and I can reach my entire network from anywhere in the country, which is more than I expected to get out of a router and a Raspberry Pi.`,
      ],
    },
    {
